...
def test_reasoning_engine_sync_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    has_content = any(
        (event.get("content") or {}).get("parts")
        and any(part.get("text") or part.get("data") or part.get("inline_data") for part in event["content"]["parts"])
        for event in events
    )
    assert has_content, "No text or data content in reasoning_engine sync events"
