...
def get_memory_service():
    """Process-wide memory service using Vertex AI Memory Bank when agent_engine_id is available."""
    if agent_engine_id := (os.environ.get("MEMORY_BANK_ID") or os.environ.get("GOOGLE_CLOUD_AGENT_ENGINE_ID")):
        from google.adk.memory import VertexAiMemoryBankService

        return VertexAiMemoryBankService(
            project=os.environ.get("GOOGLE_CLOUD_PROJECT", "qwiklabs-gcp-03-bec4fda9e582"),
            location=os.environ.get("GOOGLE_CLOUD_AGENT_ENGINE_LOCATION")
